...
const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // FastAPI UserCreate schema now accepts: email, password, role, first_name, last_name
    try {
      const response = await customFetch.post("/users", {
        email: data.email,
        password: data.password,
        role: "user",
        first_name: (data.name as string) || null,
        last_name: (data.lastname as string) || null,
      });
